  ...
  let run_split_flap text Universal.{ prefix; suffix; terminator; width }
    ...
    let buildup txt =
      ...
      let rec loop acc pos len =
        ...
        | true ->
            let lt =
              if predlen = 0 then acc else sub ~pos ~len:predlen :: acc
            in
    ...
    let pad txt =
      List.map txt ~f:(fun x ->
          ...
          | Right, false ->
              list_concat ~sep:[] [ List.init diff ~f:intspace; x ]
          | Center, false ->
              let r = diff / 2 in
              list_concat ~sep:[]
    ...
    let rec run_workers counts ~letters ~flips =
      if flips <= 0 then ()
        ...
        Bytes.From_string.unsafe_blit ~src:line ~src_pos:0 ~dst:buffer
          ~dst_pos:0 ~len:llen;
        print buffer 0 llen;
